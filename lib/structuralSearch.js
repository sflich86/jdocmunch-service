...
function extractChapterNumber(query) {
  var normalized = normalizeText(query);
  var match = normalized.match(/\b(?:chapter|capitulo)\s+(\d+)\b/);
...
function isChapterTitleQuery(query) {
  var normalized = normalizeText(query);
  return (
    /\b(?:titulo|title)\b/.test(normalized) &&
    /\b(?:chapter|capitulo)\b/.test(normalized)
  );
}
...
function searchStructuralChapterMetadata(query, metadataMap, options) {
  ...
  var chapterNumber = extractChapterNumber(query);
  if (!Number.isFinite(chapterNumber)) return [];

  var books = Object.values(metadataMap || {});
