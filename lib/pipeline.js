...
async function processIndexing(userId, bookId, jobId) {
    ...
    try {
        ...
        // Indexación MCP liviana: evitamos embeddings/summaries duplicados aquí
        // porque el refresco semántico propio corre inmediatamente después.
...
async function generateCrossBookSynthesis(userId, jobId) {
    ...
    const booksRes = await db.execute({
        sql: "SELECT id, title, author, source_language, pedagogical_compendium, context_core_json FROM books WHERE user_id = ? AND pedagogical_compendium IS NOT NULL",
        args: [userIdClean]
    });
