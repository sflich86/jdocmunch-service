...
async function processIndexing(userId, bookId, jobId) {
    console.log(`[Pipeline] 🔍 Iniciando indexación NATIVA para ${bookId}...`);
    
    try {
        const repoName = `local/${userId || 'default'}`;
        const userBooksDir = path.join(__dirname, '..', 'books', userId || 'default');

        // Llamamos a la herramienta NATIVA de jdocmunch-mcp
        const result = await callTool("index_local", {
            path: userBooksDir,
            use_ai_summaries: false, // Por velocidad, el ADN ya lo hicimos nosotros
            use_embeddings: true
        });
        
        console.log(`[Pipeline] ✓ Indexación completada:`, result.content[0].text.slice(0, 200));
    } catch (e) {
