...
const { refreshUserSemanticIndex } = require("./semanticSearch");
const { buildStructuredMarkdownFromChapters } = require("./chunkMetadata");
...
async function processIndexing(userId, bookId, jobId) {
    ...
    try {
        ...
        const content = await getStructuredIndexContent(bookId, rawContent);
        ...
        await jobLogger.log(
            ...
            'INDEXING',
            'meta',
            `Embeddings actualizados con ${semanticResult.embedding_model} para ${semanticResult.sections} secciones.`
        );
...
async function getStructuredIndexContent(bookId, content) {
    try {
        ...
        const chapters = JSON.parse(String(structureRes.rows[0].chapters || "[]"));
        return buildStructuredMarkdownFromChapters(content, chapters);
    } catch (e) {
        return content;
    }
}
...
`;
    ...
    await db.execute({
        ...
        args: [compendio, JSON.stringify(contextCoreLite), bookId]
    });
