...
async function runFullPipeline(userId, bookId, jobId) {
    ...
    try {
        ...
        for (let i = startIndex; i < STEPS.length; i++) {
            ...
            // Ejecutar la función del paso con reintentos internos si es necesario
            // pero el wrapper callGemini ya maneja la mayoría de los casos de cuota.
            await step.fn(userId, bookId, jobId);
            
            console.log(`[Pipeline] ✓ ${step.name} completado.`);
            
            // Pausa de cortesía entre pasos pesados para Gemini
            if (i < STEPS.length - 1) {
...
async function processIndexing(userId, bookId, jobId) {
    // Aquí llamamos a la lógica anterior de MCP Bridge o al indexador local.
    // Para simplificar esta etapa de refactor, mantendremos la llamada a la lógica de server.js
    // pero inyectando la protección de cuota.
    
    // NOTA: Como el indexador MCP usa su propio transporte stdio, la gestión de keys 
    // debe pasarse vía variable de entorno.
    const apiKey = await callGemini(async (k) => k, { tier: 'embedding', description: `indexing:${bookId}` });
    
    // Simulación de llamada a MCP (en la práctica se integrará con el transport del server.js)
    console.log(`[Pipeline] Preparando indexación con MCP para ${bookId}...`);
    // (Esta parte requerirá coordinación con la instancia de MCP en server.js o spawnear una nueva)
    // Por ahora, asumimos que server.js maneja la conexión MCP.
}
...
async function generateSocraticElements(userId, bookId, jobId) {
    ...
    const textProvs = await callGemini(async (apiKey) => {
        ...
        const result = await client.models.generateContent({
            model: 'gemini-2.0-flash-exp',
            contents: [{ role: 'user', parts: [{ text: promptProvs }] }]
        });
        return result.text;
