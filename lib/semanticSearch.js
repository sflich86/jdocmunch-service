...
async function searchUserIndex(query, userId, options) {
    ...
    var properNamePhrases = extractProperNamePhrases(query);
    ...
    for (var i = 0; i < indexData.sections.length; i++) {
        ...
        var lexicalHaystack = normalizeSearchText([
            section.title || "",
            section.summary || "",
        ...
        var lexicalOverlap = queryTokens.reduce(function(total, token) {
            return total + (queryTokenSet.has(token) && lexicalHaystack.indexOf(token) !== -1 ? 1 : 0);
        }, 0);
        var lexicalBonus = lexicalOverlap * 0.015;
        for (var phraseIndex = 0; phraseIndex < properNamePhrases.length; phraseIndex++) {
            if (lexicalHaystack.indexOf(properNamePhrases[phraseIndex]) !== -1) {
                lexicalBonus += 0.18;
            }
        }
