    ...
    for (var i = 0; i < indexData.sections.length; i++) {
        ...
        try {
            section.embedding = await embedText(embedTextInput, DEFAULT_DOCUMENT_TASK, section.title, env, {
                mode: "recovery",
                description: "semantic-embed:" + DEFAULT_DOCUMENT_TASK
            });
            section.embedding_model = targetModel;
            embeddedCount++;
            progressTouched = true;
            if (embeddedCount % checkpointEvery === 0) {
                writeEmbeddingCheckpoint(userId, indexData, env, targetModel, {
                    ...
                    total_sections: indexData.sections.length
                });
            }
        } catch (err) {
            writeEmbeddingCheckpoint(userId, indexData, env, targetModel, {
            ...
            err.message = err.message + " (progreso embeddings: " + embeddedCount + " nuevas, " + reusedCount + " reutilizadas, " + skippedCount + " omitidas de " + indexData.sections.length + ")";
            throw err;
        }
    }
